...
class Adapter implements AdapterInterface{
    ...
    function __construct(DocumentManager $em)
    {     
        $this->em = $em;
    ...
    public function authenticate(){
        ...
        /** @var $repository \Application\Document\UsuarioRepository*/
        $repository = $this->em->getRepository('Application\Document\Usuario');

        $usuario    = $repository->findByEmailSenha($this->getUsername(), $this->getPassword());


        /** @var $usuario \Application\Document\Usuario*/
        if($usuario){
            if($usuario->isAtivo()){
